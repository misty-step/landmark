...
## Oracle
- [x] `dist/landmark synthesize --dry-run-cost ...` reports estimated input tokens, output tokens, model tier, skip decision, and cost before any LLM call.
...
3. Introduce model policy fields in `.landmark.yml` and action inputs only where the manifest cannot cover the use case.
