...
## Oracle
- [x] The self-release PR path updates `dist/landfall` and `dist/landfall.sha256` whenever `crates/landfall/Cargo.toml` changes.
- [x] CI compares `target/x86_64-unknown-linux-musl/release/landfall` to `dist/landfall`, matching local `bin/gate`.
...
## Delivery Evidence
- Red replay: `cargo run --locked -- replay-action --evidence-dir .landfall/replay-012-red --scenario self_release_pr_path` failed with `prepare-self-release did not refresh dist/landfall`.
- Focused replay: `cargo run --locked -- replay-action --evidence-dir .landfall/replay-012-green --scenario self_release_pr_path` passed and recorded `dist/landfall` plus `dist/landfall.sha256` in the generated release plan evidence.
...
- Hosted parity fix: local macOS cross-builds still differed from Ubuntu Linux builds, so `bin/build-linux-action` now refreshes or checks `dist/landfall` inside the pinned Linux builder on non-Linux hosts, and `prepare-self-release` refuses to produce the production Linux artifact from non-Linux hosts.
...
- Artifact backfill: downloaded `landfall-linux-action-binary` from PR #118 run `27445100772`, replaced `dist/landfall`, and regenerated `dist/landfall.sha256` to `ff9be02556f7cd9b4a58f872c58e8a6e56d467295c622a3184fdd28929fd8f4c`.
