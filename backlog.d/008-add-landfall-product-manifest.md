...
## Oracle
- [x] `dist/landfall init --repo-root . --output .landfall.yml --dry-run` emits a valid manifest seeded from observable repository metadata.
- [x] `dist/landfall setup --repo-root . --output-dir .landfall/setup` reads `.landfall.yml` and reflects manifest values in generated workflows.
- [x] `dist/landfall synthesize ...` uses manifest `product`, `audience`, `voice`, artifact, and model policy defaults when equivalent action inputs are absent.
...
## Closure Evidence
- Implemented typed `.landfall.yml` schema, `init`, `doctor`, and `manifest-defaults` in the Rust CLI.
...
- Verification:
  ...
  - `cargo run --locked -- init --repo-root . --output .landfall.yml --dry-run` emitted the seeded Landfall manifest.
  - `cargo run --locked -- replay-action --evidence-dir .landfall/replay-008-manifest --scenario manifest_defaults_and_overrides` passed.
  - `cargo run --locked -- replay-action --evidence-dir .landfall/replay-008-action-defaults --scenario action_manifest_defaults_precedence` passed.
  - `bin/gate` passed and wrote `.landfall/replay/replay-result.json`; canonical replay includes `action_manifest_defaults_precedence` and `manifest_defaults_and_overrides`.
  - Local macOS cannot execute the checked-in Linux `dist/landfall`; `bin/gate` verifies checksum locally and defers byte-for-byte source-to-dist parity to hosted Linux CI.
