...
Landmark is public open-source release intelligence for repositories that use
conventional commits. It can run as a GitHub Action, but the product boundary is
the Rust CLI: local scripts, generic CI, and agents should all be able to ask
the same runtime what changed, what version follows, what evidence exists, and
...
plans, provenance, approval state, and machine-readable evidence. GitHub,
Slack, feeds, docs patches, blog drafts, screenshots, videos, and other
final-mile surfaces are adapters and producers around that truth.
...
programmatic. A release should be explainable before it mutates anything remote.
...
4. Treat schema registry coverage as part of the product, especially for agents.
