...
Landfall is a composite GitHub Action that intentionally delegates version
analysis, changelog generation, and GitHub Release creation to
`semantic-release`. Before this decision, Landfall-owned behavior lived in
...
bootstrap Python or pip for Landfall-owned behavior.

## Decision

Landfall-owned runtime behavior is implemented as a Rust CLI in
`crates/landfall` and distributed to action consumers as the checked-in Linux
binary `dist/landfall`.
...
target and `rust-lld`; `dist/landfall.sha256` is committed with it.

## Consequences

Consumers no longer need Python or pip when using Landfall. Synthesis-only mode
...
rollback boundary is the action wrapper and `dist/landfall` invocation points.

If the checked-in binary is stale or invalid, `bin/gate` fails checksum
verification. If runtime behavior drifts, replay evidence under `.landfall/`
