...
pub(crate) fn schema_descriptors() -> Vec<SchemaDescriptor> {
    vec![
        ...
        SchemaDescriptor {
            ...
            version: "v1",
            artifact: "prepared, artifact-bound, or completed release transaction",
        },
        SchemaDescriptor {
...
pub(crate) fn command_contracts() -> Vec<CommandContract> {
    vec![
        ...
        CommandContract {
            command: "release-transaction commit",
            mode: "portable-release-transaction",
            mutates: true,
            preview: "--dry-run inspects remote release state without mutating; retries verify the recorded receipt and return the same completed result",
            stdout: "ReleaseTransaction JSON with completed receipt",
            stderr: "logs and errors only",
            json_output: true,
        },
        CommandContract {
